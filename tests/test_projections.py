...
@pytest.mark.asyncio
async def test_projection_should_create_projection(
    saconnection, eventsourcing, statement
):
    # arrange
    ...
    bank_account.dispatch(BankAccountCreated(id=123, owner="John Doe"))
...
@pytest.mark.asyncio
async def test_projection_should_update_projection(
    saconnection, eventsourcing, statement
):
    # arrange
    class Statement(projections.Projection):
    ...
    bank_account_1.dispatch(BankAccountCreated(id=321, owner="John Doe"))
    bank_account_2 = BankAccount()
    bank_account_2.dispatch(BankAccountCreated(id=789, owner="John Doe"))
    async with eventsourcing.open("event_store") as eventstore:
        await eventstore.append_to_stream(
            bank_account_1.id, bank_account_1.get_events(), bank_account_1.notify_save
        )
        await eventstore.append_to_stream(
            bank_account_2.id, bank_account_2.get_events(), bank_account_2.notify_save
        )
        bank_account_1.dispatch(DepositPerformed(amount=20))
        bank_account_1.dispatch(DepositPerformed(amount=20))
        await eventstore.append_to_stream(
            bank_account_1.id, bank_account_1.get_events(), bank_account_1.notify_save
        )
...
@pytest.mark.asyncio
async def test_projection_should_raise_exception_when_update_without_primary_keys(
    saconnection, eventsourcing, statement
):
    # arrange
    class Statement(projections.Projection):
    ...
    bank_account = BankAccount()
    bank_account.dispatch(BankAccountCreated(id=456, owner="John Doe"))
    async with eventsourcing.open("event_store") as eventstore:
        await eventstore.append_to_stream(
            bank_account.id, bank_account.get_events(), bank_account.notify_save
        )
        bank_account.dispatch(DepositPerformed(amount=20))
        bank_account.dispatch(DepositPerformed(amount=20))
        # assert
